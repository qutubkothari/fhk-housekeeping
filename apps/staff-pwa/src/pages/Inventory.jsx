...
export default function Inventory({ user, lang = 'en' }) {
  ...
  const [itemsViewMode, setItemsViewMode] = useState('cards') // 'cards' or 'table' for inventory items
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-green-600 to-green-700">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center text-xs font-semibold text-white uppercase tracking-wider">Reorder</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredItems.map((item, index) => {
                  const status = getStockStatus(item)
                  return (
                    <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-green-50 transition-colors`}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-semibold text-gray-900">{item.item_name_en}</div>
                        {item.item_name_ar ? <div className="text-sm text-gray-500" dir="rtl">{item.item_name_ar}</div> : null}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800 capitalize border border-gray-200">
                          {item.category}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-700 capitalize">{item.unit}</div>
                      </td>
