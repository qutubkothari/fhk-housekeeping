...
export default function Procurement({ user, lang = 'en' }) {
  ...
  const calculateGRNTotals = () => {
    const subtotal = (grnItems || []).reduce((sum, it) => {
  ...
  const handleSubmitGRN = async (e) => {
    ...
    try {
      const hasAnyReceived = (grnItems || []).some((it) => (Number(it.received_quantity) || 0) > 0)
      const isFullyReceived = (grnItems || []).every((it) => (Number(it.received_quantity) || 0) >= (Number(it.ordered_quantity) || 0))
      ...
      // Create GRN items (auto-accept by default, editable received qty)
      const grnItemsToInsert = (grnItems || []).map((it) => {
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl">
            ...
            <form onSubmit={handleSubmitGRN} className="p-6 space-y-4">
              ...
              <p className="text-sm text-gray-600">
                Edit Qty if received differs. Default Qty auto-accepts the full ordered quantity.
              </p>
              ...
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="p-4 space-y-2">
                  ...
                  {(grnItems || []).map((it, idx) => (
                    <div key={it.invoice_item_id || idx} className="grid grid-cols-5 gap-2 items-center">
                      ...
                      <input
                        ...
                        onChange={(e) => updateGRNItem(idx, 'received_quantity', parseInt(e.target.value || '0', 10))}
                        className="px-3 py-2 border border-gray-300 rounded-lg text-sm"
                      />
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">${calculateGRNTotals().subtotal.toFixed(2)}</span>
                </div>
              </div>
